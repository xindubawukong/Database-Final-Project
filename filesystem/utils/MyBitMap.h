...
class MyBitMap {
 protected:
  ...
  uint rootMask;
  // virtual
  uint getLeafData(int index) { return data[index]; }
  // virtual
  void setLeafData(int index, uint v) { data[index] = v; }
  ...
  int _findLeftOne(int level, int offset, int pos, int prevLevelCap) {
    ...
    return _findLeftOne(level - 1, offset - prevLevelCap, nPos,
                        (prevLevelCap << BIAS));
  }
  ...
  */
  ...
  static int getIndex(uint k) { return h[_hash(k)]; }
  static uint lowbit(uint k) { return (k & (-k)); }
  static void getPos(int index, int& pos, int& bit) {
    pos = (index >> BIAS);
    bit = index - (pos << BIAS);
  }
  uint data0() { return data[0]; }
